  ...
  : TaskComposerGraph(std::move(name))
{
  ...
  // Raster planner
  auto raster_task = std::make_unique<RasterCtMotionTask>(output_keys_[0], output_keys_[0], false);
...
TaskComposerNode::UPtr RasterCtGlobalPipelineTask::clone() const
{
  return std::make_unique<RasterCtGlobalPipelineTask>(input_keys_[0], output_keys_[0], name_);
}

bool RasterCtGlobalPipelineTask::operator==(const RasterCtGlobalPipelineTask& rhs) const
{
  bool equal = true;
  equal &= TaskComposerGraph::operator==(rhs);
  return equal;
}
bool RasterCtGlobalPipelineTask::operator!=(const RasterCtGlobalPipelineTask& rhs) const { return !operator==(rhs); }
