/**
 * @file raster_ft_only_motion_task.cpp
...
{
RasterFtOnlyMotionTask::RasterFtOnlyMotionTask(std::string input_key,
...
TaskComposerNodeInfo::UPtr RasterFtOnlyMotionTask::runImpl(TaskComposerInput& input,
...
void RasterFtOnlyMotionTask::checkTaskInput(const tesseract_common::Any& input)
{
  ...
  // -------------
  if (input.isNull())
    throw std::runtime_error("RasterFtOnlyMotionTask, input is null");

  if (input.getType() != std::type_index(typeid(CompositeInstruction)))
    throw std::runtime_error("RasterFtOnlyMotionTask, input is not a composite instruction");

  const auto& composite = input.as<CompositeInstruction>();

  // Check that it has a start instruction
  if (!composite.hasStartInstruction())
    throw std::runtime_error("RasterFtOnlyMotionTask, input should have a start instruction");
  ...
  for (std::size_t index = 0; index < composite.size(); index++)
  {
    ...
    if (!composite.at(index).isCompositeInstruction())
      throw std::runtime_error("RasterFtOnlyMotionTask, Both rasters and transitions should be a composite");
  }
}
...
bool RasterFtOnlyMotionTask::operator!=(const RasterFtOnlyMotionTask& rhs) const { return !operator==(rhs); }

template <class Archive>
void RasterFtOnlyMotionTask::serialize(Archive& ar, const unsigned int /*version*/)
{
...
TESSERACT_SERIALIZE_ARCHIVES_INSTANTIATE(tesseract_planning::RasterFtOnlyMotionTask)
BOOST_CLASS_EXPORT_IMPLEMENT(tesseract_planning::RasterFtOnlyMotionTask)
