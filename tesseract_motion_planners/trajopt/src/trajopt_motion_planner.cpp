...
TrajOptMotionPlanner::createProblem(const PlannerRequest& request) const
{
  ...
  for (int i = 0; i < static_cast<Eigen::Index>(move_instructions.size()); ++i)
  {
    ...
    TrajOptWaypointInfo wp_info =
        cur_plan_profile->createCostAndConstraints(move_instruction, composite_mi, request.env, active_links, i);
  ...
  TrajOptTermInfos c_term_infos = composite_profile->createCostAndConstraints(
      composite_mi, request.env, fixed_steps, 0, pci->basic_info.n_steps - 1);
