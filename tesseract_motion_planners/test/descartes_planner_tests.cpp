...
TEST_F(TesseractPlanningDescartesUnit, DescartesPlannerCollisionEdgeEvaluator)  // NOLINT
{
  ...
  // Create a seed
  CompositeInstruction interpolated_program = generateSeed(program, cur_state, env_, 3.14, 1.0, 3.14, 2);
...
TEST_F(TesseractPlanningDescartesUnit, DescartesPlannerFixedPosesLegacy)  // NOLINT
{
  // Create the planner and the responses that will store the results
  PlannerResponse planning_response;
  ...
  // Create Planner
  DescartesLegacyMotionPlannerD single_descartes_planner;
  plan_profile->num_threads = 1;
...
TEST_F(TesseractPlanningDescartesUnit, DescartesPlannerAxialSymetricLegacy)  // NOLINT
{
  // Create the planner and the responses that will store the results
  PlannerResponse planning_response;
...
TEST_F(TesseractPlanningDescartesUnit, DescartesPlannerCollisionEdgeEvaluatorLegacy)  // NOLINT
{
  // Create the planner and the responses that will store the results
  PlannerResponse planning_response;
  ...
  // Create a seed
  CompositeInstruction seed = generateSeedLegacy(program, cur_state, env_, 3.14, 1.0, 3.14, 2);
