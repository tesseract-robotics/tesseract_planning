...
TYPED_TEST(OMPLTestFixture, OMPLFreespacePlannerUnitLegacy)  // NOLINT
{
  EXPECT_EQ(ompl::RNG::getSeed(), SEED) << "Randomization seed does not match expected: " << ompl::RNG::getSeed()
                                        << " vs. " << SEED;
  ...
  // Create a seed
  CompositeInstruction seed = generateSeedLegacy(program, cur_state, env, 3.14, 1.0, 3.14, 10);
  ...
  // Create Planner and solve
  OMPLLegacyMotionPlanner ompl_planner;
  PlannerResponse planner_response = ompl_planner.solve(request);
  ...
  // Create a new seed
  seed = generateSeedLegacy(program, cur_state, env, 3.14, 1.0, 3.14, 10);
...
TYPED_TEST(OMPLTestFixture, OMPLFreespaceCartesianGoalPlannerUnitLegacy)  // NOLINT
{
  EXPECT_EQ(ompl::RNG::getSeed(), SEED) << "Randomization seed does not match expected: " << ompl::RNG::getSeed()
                                        << " vs. " << SEED;
...
TYPED_TEST(OMPLTestFixture, OMPLFreespaceCartesianStartPlannerUnitLegacy)  // NOLINT
{
  EXPECT_EQ(ompl::RNG::getSeed(), SEED) << "Randomization seed does not match expected: " << ompl::RNG::getSeed()
                                        << " vs. " << SEED;
